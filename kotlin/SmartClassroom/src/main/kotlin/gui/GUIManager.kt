...
//    private val addUserPanel: JPanel = JPanel(BorderLayout())
    ...
    private fun grabSerialBus() {
        context.serialManager!!.addDataReceivedListener {
            it.forEach { (param, value) ->
                when (param) {
                    SerialManager.LABEL_SYSTEM -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_ECHO -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_ERROR -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_CONFIG -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_MAP -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_UID -> authorisation(uid = context.serialManager!!.get(SerialManager.LABEL_UID)!!)
                    SerialManager.LABEL_LIGHT -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_WINDOW -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_POWER_SUPPLY -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_STARTUP -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_SAVE -> println("$param - ${context.serialManager!!.get(param)!!}")
                    SerialManager.LABEL_LOAD -> println("$param - ${context.serialManager!!.get(param)!!}")
                }
            }
    ...
    init {
        ...
        settingsRootPanel.actionContent = arrayOf(Labels.PORT_CONNECT, Labels.SERIAL_LOG)
        settingsUserPanel.actionContent = arrayOf(Labels.PORT_CONNECT, Labels.SERIAL_LOG)
